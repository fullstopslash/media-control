...
| 2026-05-03T16:58Z | **Intent-017 daemon-stop hang RESOLVED INCIDENTALLY by bolt 030** | Measured `time systemctl --user stop media-control-daemon.service` post-018: **0.004s** wall time (vs. prior 90s+ ending in `State 'stop-sigterm' timed out. Killing.`). The intent-017 hypothesis (FIFO `File::open` mid-await preventing AbortOnDrop from reaping in bounded time) was correct — bolt 030's deletion of `fifo_listener` resolved the hang for free. **No follow-up intent needed.** | Verified live on `malphas` |
| 2026-05-03T17:00Z | Intent 018 closed by bolt-complete cascade | Both units (001-socket-transport, 002-rollout-migration) and both bolts (030, 031) marked complete. Intent status: `inception-complete → complete`. | bolt-complete.cjs |
