...
impl JellyfinClient {
    ...
    ///
    /// The response body is explicitly drained so the underlying TCP connection
    /// can be returned to the connection pool. Without this, reqwest keeps the
    /// connection open waiting for the body to be consumed, which starves the
    ...
    async fn post_empty(&self, path: &str, query: &[(&str, &str)]) -> Result<()> {
        let resp = self
            ...
            .await?
            .error_for_status()?;
        let _ = resp.bytes().await; // drain body to release connection to pool
        Ok(())
    }
    ...
    /// # Known limitation
    ///
    /// Playback info (transcode profile, audio/subtitle stream indices, media
    /// source ID) is fetched only for `item_ids[0]` and applied to the entire
    /// queue. Items at positions 1+ may have different media characteristics
    /// (codec, stream layout, etc.) that would ideally use their own playback
    /// info. Per-item fetching is not done here because the Jellyfin play
    /// command accepts a single `PlaySessionId`/`MediaSourceId` for the whole
    /// batch; the server re-evaluates transcode params as each item starts.
    // FIXME: transcode profile is derived from item[0] only — items 1+ may
    // have different codec/stream characteristics. The server re-evaluates on
    // each item start, but audio/subtitle index hints sent here could be wrong
    ...
    pub async fn play_items(&self, session_id: &str, item_ids: Vec<String>) -> Result<()> {
        ...
        // Playback info is fetched for the first item only; see FIXME above.
        warn!(
            first_item = %item_ids[0],
            queue_len = item_ids.len(),
            "play_items: transcode profile derived from first item only; \
             audio/subtitle stream hints may be incorrect for subsequent items"
        );
