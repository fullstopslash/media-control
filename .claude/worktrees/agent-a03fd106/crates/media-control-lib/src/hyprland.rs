...
impl HyprlandClient {
    ...
    ///
    /// Hyprland may return `"ok\r\n"` (CRLF line endings), so each line is
    /// trimmed before comparison to handle both LF and CRLF responses.
    #[inline]
    fn is_success(response: &str) -> bool {
        response.is_empty()
            || response
                .lines()
                .all(|line| line.trim().is_empty() || line.trim() == "ok")
    }
