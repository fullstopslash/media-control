...
impl Config {
    ...
    ///
    /// Also applies the same NFA size cap used by [`WindowMatcher`] at runtime
    /// so a pattern that would be rejected there is caught here at load time.
    /// Rejects empty `value` fields (an empty regex matches every window, which
    /// is almost certainly unintentional) and warns on unrecognised `key` fields.
    fn validate_pattern_regexes(patterns: &[Pattern]) -> Result<()> {
        use crate::window::PATTERN_REGEX_SIZE_LIMIT;

        /// Recognised `key` values for a `[[patterns]]` entry.
        const KNOWN_KEYS: &[&str] = &["class", "title"];

        for p in patterns {
            if p.value.is_empty() {
                return Err(ConfigError::InvalidValue {
                    field: "patterns[].value",
                    reason: "must not be empty (an empty regex matches every window)".to_string(),
                });
            }
            if !KNOWN_KEYS
                .iter()
                .any(|k| k.eq_ignore_ascii_case(&p.key))
            {
                tracing::warn!(
                    key = %p.key,
                    "unknown pattern key in config — pattern will be skipped at runtime"
                );
            }
